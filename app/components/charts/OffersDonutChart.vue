<template>
  <div class="relative w-full max-w-[395px] bg-[#EFF3FA] overflow-hidden rounded-2xl shadow-sm p-4 px-[20px] pb-20 max-md:max-w-full">
    ...
    <div class="flex items-start gap-5">
      <div class="space-y-5">
        <div v-for="(label, i) in chartData.labels" :key="label" 
          ...
          <span class="w-2 h-2 block mb-1 rounded-full" :style="{ background: chartData.colors[i] }"></span>
...
// Handle label clicks to navigate to respective pages
const handleLabelClick = (label) => {
  if (label.toLowerCase() === 'accepted') {
    navigateTo('/accepted');
